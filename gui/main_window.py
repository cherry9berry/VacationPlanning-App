...
class MainWindow:
    ...
    def insert_instructions(self):
        ...
        instructions_text = """:
СОЗДАНИЕ ФАЙЛОВ ОТПУСКОВ:
- Для генерации индивидуальных файлов отпусков подготовьте Excel-файл со списком сотрудников. Убедитесь, что файл содержит все необходимые данные и обязательные колонки.
- Выберите файл со списком сотрудников и укажите целевую папку, в которой будет создана структурированная иерархия папок по подразделениям с файлами каждого сотрудника.
- Нажмите кнопку "Создание файлов" для запуска процесса. Система автоматически пропустит уже существующие файлы, выполнив "долив" новых сотрудников и создав новые подразделения при необходимости.

РАБОТА С ОТЧЕТАМИ:
- Выберите корневую папку, которая содержит папки подразделений с созданными ранее файлами отпусков сотрудников.
- Для формирования отчета по каждому отделу: перейдите во вкладку "По подразделениям", выберите нужные подразделения из списка и нажмите кнопку "Создать отчеты по подразделениям". Отчеты будут сохранены в папках соответствующих отделов.
- Для создания общего сводного отчета по всем подразделениям: перейдите во вкладку "Общий", укажите целевую папку с подразделениями и нажмите кнопку "Создать общий отчет".

ВАЖНО:
- Все используемые Excel-шаблоны (.xlsx) должны располагаться в папке 'templates/', находящейся рядом с основным исполняемым файлом программы. Отсутствие или некорректное расположение шаблонов приведет к отключению соответствующего функционала.
- Общий отчет формируется исключительно на основе уже созданных отчетов по блокам. Если отчет по блоку отсутствует или поврежден в папке какого-либо подразделения, данные по этому подразделению не будут включены в общий отчет.
- Отчет по блоку также может быть сформирован исполняемым exe скриптом, расположенным внутри соответствующей папки блока.
- Вся подробная информация о ходе выполнения операций, возможных предупреждениях и ошибках будет выводиться непосредственно в окне текущей операции.
"""
